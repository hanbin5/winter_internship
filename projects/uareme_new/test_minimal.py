...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        for img_path in img_paths:
            ...
            # Kappa 추출
            pred_kappa = pred_norm[:, 3:, lrtb[2]:lrtb[2]+orig_H, lrtb[0]:lrtb[0]+orig_W]

            # 시각화용 저장
            target_path = img_path.replace('/img/', '/output/').replace(ext, '_kappa.png')

            pred_kappa = pred_kappa.detach().cpu().numpy()
            pred_kappa = pred_kappa[0, 0, :, :]  # (H, W)

            # normalize to [0, 255]
            pred_kappa_vis = (pred_kappa - pred_kappa.min()) / (pred_kappa.max() - pred_kappa.min() + 1e-8)
            pred_kappa_vis = (pred_kappa_vis * 255).astype(np.uint8)

            im = Image.fromarray(pred_kappa_vis)
            im.save(target_path)
            

            # # RGB
            # pred_norm = pred_norm[:, :3, lrtb[2]:lrtb[2]+orig_H, lrtb[0]:lrtb[0]+orig_W]
